...
Next, the packet is passed to the PIE algorithm, which uses a drop probability to determine whether the packet should be enqueued or dropped, as outlined in {{!RFC8033}}. This drop probability is updated periodically (every 16 ms, as per {{!RFC8033}}) based on the current queue delay’s deviation from the target delay and whether the delay is trending up or down.
...
It is important to note that the timestamping approach provides a "per-packet queue delay," while the drop probability is calculated periodically (every 16 ms, as specified in {{!RFC8033}}). Therefore, the FQ-PIE algorithm MAY use the queue delay value from the most recently dequeued packet when calculating the drop probability.
